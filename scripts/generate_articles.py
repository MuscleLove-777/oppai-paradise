...
{{ actresses }}さんが出演するこの作品。巨乳好きなら見逃せない一本です。
{% endif %}

{% if maker %}
> **{{ maker }}**からリリースされたこの作品は、巨乳・美乳ジャンルで定評があります。
{% endif %}
...
INTRO_VARIATIONS = [
    "たわわに揺れる...！**「{title}」**は{genre_text}好きにはたまらない一本です。",
    "巨乳好きにはたまらない...！**「{title}」**、{genre_text}好きなら絶対ハマる作品が来ました。",
    "{genre_text}で興奮したいならコレ！**「{title}」**がマジでおすすめ。",
    "話題沸騰中の**「{title}」**をピックアップ。{genre_text}のド直球おっぱい作品です。",
    "**「{title}」**が気になってる人、正解です。{genre_text}ジャンルの中でもガチで抜ける巨乳作品。",
    "本日の厳選巨乳作品は**「{title}」**。{genre_text}好きの間で「神作品」と話題に。",
    "新着から見つけた掘り出し物！**「{title}」**、{genre_text}がたっぷり詰まった至高の一本です。",
    "サンプル動画だけでも興奮必至！**「{title}」**は{genre_text}の最高傑作かも。",
    "今週一番シコれる巨乳作品はコレ。**「{title}」**、{genre_text}好きは見逃すな！",
    "MuscleLoveが厳選！**「{title}」**は{genre_text}好きなら見逃せない巨乳作品です。",
    "MuscleLove編集部おすすめの**「{title}」**。{genre_text}ジャンルで今一番アツい巨乳作品。",
    "MuscleLoveイチオシ！**「{title}」**、{genre_text}好きを唸らせる最高の巨乳作品が来ました。",
]
...
def _generate_single_article(
    ...
    # タグの生成
    tag_list = genres[:5] if genres else ["巨乳", "美乳"]
    tags = ", ".join(f'"{t}"' for t in tag_list)

    # ジャンルテキスト（導入文用）
    genre_text = "・".join(genres[:3]) if genres else "巨乳"
...
def _build_meta_description(title: str, genre_text: str, actresses: str) -> str:
    ...
    desc_variations = [
        f"{title}のサンプル動画・レビュー。巨乳・美乳の{genre_text}系作品を紹介。",
        f"巨乳好き必見の「{title}」を徹底レビュー。{genre_text}好きにおすすめのおっぱいAV。",
        f"ボリュームMAXの{genre_text}作品「{title}」。巨乳シチュエーションに興奮必至。",
        f"巨乳作品の注目作「{title}」。{genre_text}のおっぱいシーンをサンプル動画付きで紹介。",
    ]
    if actresses:
        desc_variations.append(
            f"{actresses}出演「{title}」。{genre_text}系巨乳・美乳動画をチェック。"
        )
...
def _build_alt_text(title: str, actresses: str, genre_text: str) -> str:
    ...
    alt_variations = [
        f"巨乳・美乳作品「{title}」のパッケージ画像",
        f"「{title}」{genre_text}系巨乳作品のサムネイル",
        f"たわわ感がたまらない「{title}」の作品画像",
    ]
    if actresses:
        alt_variations.append(f"{actresses}出演「{title}」巨乳・美乳作品の画像")
...
"""
    for idx, img_url in enumerate(images, 1):
        gallery_html += f'  <img src="{img_url}" alt="巨乳・美乳作品のサンプル画像{idx}" style="width: 100%; border-radius: 4px;" loading="lazy" />\n'
